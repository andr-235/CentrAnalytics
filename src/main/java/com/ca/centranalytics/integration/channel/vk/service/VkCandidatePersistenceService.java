    ...
    public VkUserCandidate upsertUserCandidate(VkUserCandidate candidate) {
        return vkUserCandidateRepository.findByVkUserId(candidate.getVkUserId())
                .map(existing -> {
                    ...
                    existing.setUsername(candidate.getUsername());
                    existing.setProfileUrl(candidate.getProfileUrl());
                    existing.setCity(candidate.getCity());
                    existing.setHomeTown(candidate.getHomeTown());
                    existing.setBirthDate(candidate.getBirthDate());
                    existing.setSex(candidate.getSex());
                    existing.setStatus(candidate.getStatus());
                    existing.setLastSeenAt(candidate.getLastSeenAt());
                    existing.setAvatarUrl(candidate.getAvatarUrl());
                    existing.setMobilePhone(candidate.getMobilePhone());
                    existing.setHomePhone(candidate.getHomePhone());
                    existing.setSite(candidate.getSite());
                    existing.setRelation(candidate.getRelation());
                    existing.setEducation(candidate.getEducation());
                    existing.setCareerJson(candidate.getCareerJson());
                    existing.setCountersJson(candidate.getCountersJson());
                    existing.setRegionMatchSource(resolveMatchSource(existing.getRegionMatchSource(), candidate.getRegionMatchSource()));
                    existing.setCollectionMethod(candidate.getCollectionMethod());
                    existing.setRawJson(candidate.getRawJson());
